...
import { Avatar, Button, Input, List, Modal, Rate } from "antd";
...
const Details = () => {
  ...
  // Rating
  function saveRating(id, newRating) {
    // console.log(id);
    let rating = {
      rating: newRating,
      name: currentUser,
    ...
    // fetching data for Math AVERAGE
    // getFetchPorducts() {
    //   this.setState({
    //       loading: true
    //   }, () => {
    ...
    // }
    // }
    let userRatings = oneProduct.ratings.some(
      (item) => item.user === currentUser
    );
    if (userRatings) {
      let filteredRatings = oneProduct.ratings.filter((item) => {
        return item.user !== currentUser;
      });
      updateRating(params.id, filteredRatings);
    } else {
      let ratings = [...oneProduct.ratings, rating];
      ...
      <div className="details-container-inner">
        ...
        <div className="details-context">
          ...
          <Rate
            ...
            defaultValue={2.5}
            // value={ratingChanged}
            onChange={(e) => saveRating(oneProduct.id, e)}
          />
          {/* rating ends */}
          <br />
          {currentUser ? (
          ...
          {/* // Rating */}
          <div>
            <h2 style={{ marginTop: "10px" }}>Rating of the product</h2>
            <List
              className="details-comments"
              itemLayout="horizontal"
              dataSource={oneProduct.ratings}
              renderItem={(item) => (
                <List.Item
                  className="details-comments-inner"
                  style={{ marginLeft: "5px" }}
